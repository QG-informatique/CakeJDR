...
const SUN_SVG =
  'https://res.cloudinary.com/dz6ugwzxp/image/upload/v1754686032/sun-svgrepo-com_bigc5w.svg'
const MOON_SVG =
  'https://res.cloudinary.com/dz6ugwzxp/image/upload/v1754686041/moon-svgrepo-com_bn0gra.svg'
...
export default function SpecialBackground() {
    ...
    return pts.map((p, i) => {
      ...
      return (
        <div
          ...
          style={{
            position: 'absolute',
            left: `${clamp(p.x, 0, 98)}vw`,
            top: `${clamp(p.y, 56, 72.2)}vh`,
            zIndex: 6,
            pointerEvents: 'none',
          }}
        >
  ...
  const shells = useMemo(() => {
    ...
    const forms = [
      ...
      (c: string, s: string) => (
        <>
          <path
            d="M1 10 C6 1,14 1,19 10 Z"
            fill={c}
            stroke={s}
            strokeWidth="0.8"
          />
          <path
            d="M5 9 C8 4,12 4,15 9"
            fill="none"
            stroke={s}
            strokeWidth="0.6"
            opacity="0.6"
      ...
      (c: string, s: string) => (
        <>
          <path
            d="M2 10 Q10 0 18 10 Q10 12 2 10 Z"
            fill={c}
            stroke={s}
            strokeWidth="0.8"
          />
    ...
    return Array.from({ length: 22 }).map((_, i) => {
      ...
      const col = mix(
        base[Math.floor(rng() * base.length)]!,
        tints[Math.floor(rng() * tints.length)]!,
        0.18 + rng() * 0.22,
      )
  ...
  const countAdultsPlain = (list: PlainAnimal[]) =>
    list.filter((a) => !a.isBaby).length
  const adultsCountCombined = (arr: ShoreAnimal[]) =>
    arr.filter((a) => !a.isBaby).length
  const hasCrabAdult = (arr: ShoreAnimal[]) =>
    arr.some((a) => !a.isBaby && a.kind === 'crab')
      ...
      if (acc >= TICK_MS) {
        ...
        setDebris((prev) => {
          ...
          if (newLeafBubbles.length) {
            setLeafBubbles((prevB) => {
              const merged = [
                ...prevB.filter((b) => b.until > performance.now()),
                ...newLeafBubbles,
              ]
        ...
        if (performance.now() >= nextWaveAt.current) {
          ...
          setWaves((prev) => {
            const arr = [
              ...prev,
              { id: waveId.current++, x, y, until: performance.now() + 1600 },
            ]
        ...
        setAnimals((prev) => {
          ...
          for (const a of arr) {
            const shouldStay =
              (a.phaseTag === 'day' && isDayNow) ||
              (a.phaseTag === 'night' && !isDayNow)
            ...
            if (a.state === 'walk') {
              if (
                !a.target ||
                Math.hypot(a.x - a.target.x, a.y - a.target.y) < 1.2 ||
                Math.random() < 0.002
              ) {
                a.target = randomTargetPlain()
                if (Math.random() < 0.15) a.state = 'idle'
              }
          ...
          for (let i = 0; i < arr.length; i++)
            for (let j = i + 1; j < arr.length; j++) {
              ...
              if (
                (A.cooldownMateUntil ?? 0) > nowMs ||
                (B.cooldownMateUntil ?? 0) > nowMs
              )
                continue
              ...
              if (d < 2.3 && Math.random() < 0.008) {
                ...
                newHearts.push({
                  id: (A.id * 10000 + B.id) ^ 0x9e3779b9,
                  x: cx,
                  y: cy - 2,
                  until: nowMs + HEART_MS,
                })
          ...
          if (newHearts.length) {
            ...
            setHearts((prevH) =>
              prevH.filter((h) => h.until > performance.now()),
            )
          }
        ...
        setShore((prev) => {
          ...
          if (
            adultsCountCombined(arr) < MAX_SHORE_ADULTS &&
            Math.random() < 0.008
          ) {
            const forceCrab =
              !hasCrabAdult(arr) &&
              performance.now() - lastCrabCheckAt.current > 10000
          ...
          for (const a of arr) {
            ...
            const targetX =
              a.x + a.dir * (a.zone === 'water' ? a.speedWater : a.speedSand)
            const targetY =
              a.zone === 'water'
                ? a.kind === 'crab'
                  ? 77.5 + Math.sin(performance.now() / 700 + a.id) * 0.8
                  : 88 + Math.sin(performance.now() / 850 + a.id) * 0.6
                : a.kind === 'crab'
                  ? 78 + Math.sin(performance.now() / 900 + a.id) * 0.5
                  : 78 + Math.sin(performance.now() / 1050 + a.id) * 0.4

            const dx = targetX - a.x,
              dy = targetY - a.y
            const d = Math.hypot(dx, dy) || 1e-6
            const sp = a.zone === 'water' ? a.speedWater : a.speedSand
            const vx = (dx / d) * sp * 0.8,
              vy = (dy / d) * sp * 0.8
            ...
            if (Math.random() < 0.0012)
              a.zone = a.zone === 'water' ? 'sand' : 'water'
          ...
          if (newSplashes.length) {
            ...
            setSplashes((prevS) =>
              prevS.filter((s) => s.until > performance.now()),
            )
          }
          ...
          for (let i = 0; i < arr.length; i++)
            for (let j = i + 1; j < arr.length; j++) {
              ...
              if (d < 2.1 && Math.random() < 0.016) {
                const cx = (A.x + B.x) / 2,
                  cy = (A.y + B.y) / 2
                A.cooldownMateUntil = nowMs + 90000
                B.cooldownMateUntil = nowMs + 90000
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden -z-10">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to bottom, ${skyTop} 55%, ${skyBottom} 100%)`,
          zIndex: 0,
        }}
      />
      <div
        ...
        <motion.div
          ...
          animate={{
            opacity:
              starStrength === 0
                ? 0
                : [0.1 * starStrength, 0.8 * starStrength, 0.1 * starStrength],
          }}
        ...
        >
          <div
            style={{
              ...
              inset: -22,
              borderRadius: '50%',
              background: `radial-gradient(${PAL.moonHalo}99, ${PAL.moonHalo}00 70%)`,
              filter: 'blur(10px)',
            }}
          />
        ...
        >
          <div
            style={{
              ...
              inset: -20,
              borderRadius: '50%',
              background: `radial-gradient(${PAL.sunHalo}88, ${PAL.sunHalo}00 70%)`,
              filter: 'blur(8px)',
            }}
          />
      ...
      <div
        ...
        style={{
          ...
          backgroundSize:
            '100% 100%, 20px 20px, 6px 6px, 8px 8px, 22px 22px, 80px 60px',
      ...
      <div
        ...
        style={{
          ...
          backgroundSize:
            '100% 100%, 100% 10px, 10px 10px, 14px 14px, 22px 22px',
      ...
      {debris.map((d) => (
        ...
        >
          <svg viewBox="0 0 24 12" width={d.size} height={(d.size * 12) / 24}>
            <path
              d="M2 6 Q10 -2 22 6 Q10 14 2 6 Z"
              fill="#8AD18E"
              stroke="#4E9B63"
              strokeWidth="1.1"
            />
      ...
      {/* ANIMAUX PLAINE */}
      {animals.map((a) => {
        const babyFilter = a.isBaby
          ? 'saturate(0.85) brightness(1.18) hue-rotate(10deg)'
          : 'none'
        return (
          <div
            key={`animal-${a.id}`}
        ...
        return (
          ...
          >
            <img
              ...
              alt="turtle"
              style={{
                width: 34 * s.sizeScale,
                height: 'auto',
                transform: `scaleX(-1)${extra}`,
                filter: `drop-shadow(0 2px 4px #0003) ${babyFilter}`,
              }}
            />
          </div>
      ...
      {hearts.map((h) => {
        ...
        return (
          <motion.div
            key={`heart-pl-${h.id}-${Math.round(h.until)}`}
            style={{
              position: 'absolute',
              left: `${h.x}vw`,
              top: `${h.y}vh`,
          ...
          >
            <svg viewBox="0 0 32 28" width={18} height={(18 * 28) / 32}>
              <path
                d="M16 26 C-10 10 8 -2 16 8 C24 -2 42 10 16 26 Z"
                fill="#ff5a8a"
                stroke="#ff2d6b"
                strokeWidth="1"
              />
      ...
      {shoreHearts.map((h) => {
        ...
        return (
          <motion.div
            key={`heart-sh-${h.id}-${Math.round(h.until)}`}
            style={{
              position: 'absolute',
              left: `${h.x}vw`,
              top: `${h.y}vh`,
