...
// --- Helpers pips ---
function pipPattern(v: number): { r: number; c: number }[] {
  const c = { r: 1, c: 1 }
  const corners = [
    { r: 0, c: 0 },
    { r: 0, c: 2 },
        ...
        return (
          <div
            ...
            style={{
              ...
              boxShadow:
                '0 0 4px rgba(255,255,255,0.4), inset 0 0 3px rgba(0,0,0,0.7)',
...
export default function Login({ onLogin }: { onLogin: (p: string) => void }) {
  ...
  const onPointerUp = () => {
    ...
    const decay = () => {
      ...
      if (
        ...
        return
      setRotation((r) => ({
        x: r.x - velocity.current.y,
        y: r.y + velocity.current.x,
      }))
  ...
  return (
    ...
    >
      <div
        style={{ position: 'relative', width: CUBE_SIZE, height: CUBE_SIZE }}
      >
        {/* Logo au-dessus */}
        <div
          style={{
            ...
            {faces.map(({ rotX, rotY, type, value }) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: '16px',
                  boxShadow:
                    '0 4px 10px rgba(0,0,0,0.6), inset 0 0 12px rgba(214,51,108,0.15)',
