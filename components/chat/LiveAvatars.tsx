...
export default function LiveAvatars() {
  ...
  useEffect(() => {
    if (profile) {
      updateMyPresence({ pseudo: profile.pseudo, color: profile.color })
    }
  }, [profile, updateMyPresence])

  const avatars: Array<{ pseudo: string; color: string }> = []
  if (self?.presence?.pseudo) {
    avatars.push({ pseudo: self.presence.pseudo as string, color: self.presence.color as string })
  }
  others.forEach(o => {
    const p = o.presence as { pseudo?: string; color?: string } | undefined
    if (p?.pseudo) avatars.push({ pseudo: p.pseudo, color: p.color as string || '#888' })
  })
